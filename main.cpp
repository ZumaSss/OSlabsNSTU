...
LRESULT CALLBACK SoftwareMainProcedure(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp) {
	...
	case WM_COMMAND:
		...
		case OnButtonClickedClose:
			...
			//	hWnd = FindWindow(NULL, _T("Child process"));
			//	if (IsWindow(hWnd)) {
			//		SendMessage(hWnd, WM_DESTROY, SC_CLOSE, 0);
			//		
			//	}
			//	//записывать дескрипторы в массив, использовать при закрытии дескрипторы, поменять переменную bcreateprocess
			//	else {
